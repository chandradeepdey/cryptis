...
**Relational layer** (ReLoC; `core/rel.v`, `core/rel_inv_updates.v`, `primitives/*_spec.v`, `rel_adequacy.v`): `PUB⟨t, t'⟩` relates a left-run term to a right-run term — structurally, or via an *honest link* (`publicly_linked`, created with `public_rel_extend`) where the payloads may differ. Honest seal/hash links carry the tag's relational predicate `seal_pred_rel F N Φ` / `hash_pred_rel N Ψ`, whose inputs are `Some (sk, untagged payload)` per ciphertext side or `None`; decryptors recover it with `wf_seal_rel_elim` and exclude foreign tags with the leftover `seal_pred_rel_token` (use `seal_pred_rel_set_point` when every tag must be excluded). `cryptis_ctx_refinement` / `cryptis_rel_adequacy` close a game from `cryptis_rel_ctx -∗ tokens ={⊤}=∗ □ ∀ c c', channel_rel c c' -∗ REL f c << f' c' : A`; the theorems at the end of `examples/ind_cpa.v` are the template.
